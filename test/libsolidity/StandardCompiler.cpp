...
BOOST_AUTO_TEST_CASE(ethdebug_debug_info_ethdebug)
{
	...
	result = compiler.compile(generateStandardJson(false, Json::array({"ethdebug"}), Json::array({"ethdebug"})));
	BOOST_REQUIRE(result["errors"].size() == 1);
	// - via-ir enabled, everything is fine:
	result = compiler.compile(generateStandardJson(true, Json::array({"ethdebug"}), Json::array({"ethdebug"})));
	...
	result = compiler.compile(generateStandardJson(false, Json::array({"ethdebug"}), Json::array({"ir", "irOptimized", "ethdebug"})));
	BOOST_REQUIRE(result["errors"].size() == 1);
	result = compiler.compile(generateStandardJson(true, Json::array({"ethdebug"}), Json::array({"ir", "irOptimized", "ethdebug"})));
	BOOST_REQUIRE(result.contains("contracts"));
	BOOST_REQUIRE(result.dump().find("/// ethdebug: enabled") != std::string::npos);
...
BOOST_AUTO_TEST_CASE(ethdebug_ethdebug_output)
{
	...
	Json result = compiler.compile(generateStandardJson(true, std::nullopt, Json::array({"ethdebug", "ir"})));
	BOOST_REQUIRE(result.contains("contracts"));
	BOOST_REQUIRE(result.dump().find("/// ethdebug: enabled") != std::string::npos);
	...
	result = compiler.compile(generateStandardJson(false, Json::array({"all"}), Json::array({"ethdebug"})));
	BOOST_REQUIRE(result["errors"].size() == 1);
	// strict-assembly will always work e.g. solc --strict-assembly <yul> --ethdebug
	result = compiler.compile(generateStandardJson(false, std::nullopt, Json::array({"ethdebug"}), YulCode()));
	BOOST_REQUIRE(result.contains("contracts"));
	BOOST_REQUIRE(result.dump().find("ethdebug") != std::string::npos);
}
