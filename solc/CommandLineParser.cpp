...
void CommandLineParser::processArgs()
{
	...
	if (m_options.compiler.outputs.ethdebug)
	{
		...
		else
		{
			if (!m_options.output.debugInfoSelection->ethdebug)
				solThrow(
					CommandLineValidationError,
					"--debug-info must contain ethdebug, when compiling with --" +
					CompilerOutputs::componentName(&CompilerOutputs::ethdebug) + "."
				);
		}
	}
	...
	if (
		...
		(!(m_options.compiler.outputs.ir || m_options.compiler.outputs.irOptimized || m_options.compiler.outputs.ethdebug) || incompatibleEthdebugOutputs)
	)
		solThrow(
			CommandLineValidationError,
